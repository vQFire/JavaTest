...
public class MariaDB {
    ...
    public MariaDB (Connection connection) throws SQLException {
        statement = connection.createStatement();

        initializeDB();
        System.out.println("MariaDB has been inialized");
        insertTest();
        insertEncryptTest();
        selectTest();
        selectDecryptTest();
    ...
    private void insertTest () {
        try (ProgressBar pb = new ProgressBar("MARIA|INSERT", Main.MAX_INSERT * 3L)) {
    ...
    private void insertEncryptTest () {
        try (ProgressBar pb = new ProgressBar("MARIA|INSERT|CRYPT", Main.MAX_INSERT)) {
            Faker faker = new Faker();
            pb.setExtraMessage("Users");
            for (int x = 0; x < Main.MAX_INSERT; x++) {
                String SQL = String.format("""
    ...
    private void selectTest () {
        try (ProgressBar pb = new ProgressBar("MARIA|SELECT", Main.MAX_SELECT * 2L)) {
            Faker faker = new Faker();
    ...
    private void selectDecryptTest () {
        try (ProgressBar pb = new ProgressBar("MARIA|SELECT|DCRYPT", Main.MAX_SELECT)) {
            Faker faker = new Faker();
